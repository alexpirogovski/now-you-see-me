...
The extension supports three states: **Idle**, shown when ChatGPT is inactive; **Thinking**, shown primarily when ChatGPT displays its visible pulsing activity indicator, with its expandable Thinking activity control as a fallback; and **Answering**, shown while the Stop button indicates active response generation. State priority is: visible pulsing activity indicator → Thinking; otherwise Thinking control present → Thinking; otherwise Stop button present → Answering; otherwise → Idle.

The pulsing activity indicator must be visible. The Thinking-control fallback inspects only expandable buttons whose visible text consists entirely of one or more `Thinking` words. Answering detection uses the semantic Stop button, `button[aria-label="Stop"]`. Detection relies on observable ChatGPT DOM semantics and may need maintenance if ChatGPT changes its UI. The extension deliberately does not try to detect every possible internal or tool activity state.
